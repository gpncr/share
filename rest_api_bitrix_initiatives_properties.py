...
if __name__ == "__main__":
    ...
    property_name  = sys.argv[3]  if 3 < len(sys.argv)  else r'Product_Description_markets'         #sys.argv[3]
        ...
        if {'CODE', 'DISPLAY_VALUES_FORM'}.issubset(fields_df.columns):  
            ...
            for column in elements_df:
                if column in property_df['CODE'].values:
                    elements_df[column]= elements_df[column].map(property_df['DISPLAY_VALUES_FORM'].loc[property_df['CODE']==column].values[0])
        return elements_df
    ...
    def much_worse_property_handler(property_df,property_name ):
        property_df = pd.concat([property_df, 
                                 pd.DataFrame(property_df[property_name].apply(lambda x: pd.Series(x.split('|'))),index=property_df.index)], axis=1).\
                                 drop(property_name, axis=1)
...
#            'Product_needle'                     : property_handler,
#            'Product_cost_approval_razrab'       : property_handler,
#            'Product_Description_markets'        : property_handler,
#            'Product_Economic_Calculation'       : property_handler,
#            'Product_Economic_Calculation_razrab': property_handler,
#            'Product_cost_approval'              : property_handler,
